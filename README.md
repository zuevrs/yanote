...
3. **Прогоните analyzer по OpenAPI и событиям.**
   ...
   Если вам нужен не HTTP/OpenAPI path, а widened async/combined surface, не смешивайте её с HTTP numerators: отдельный guide [`docs/guides/asyncapi-kafka.md`](docs/guides/asyncapi-kafka.md) ведёт по трём доказуемым семействам — Kafka bundle `.yanote-ci/live-kafka-proof/` / `build-and-test-artifacts/live-kafka-proof/`, первому RabbitMQ/AMQP bundle `.yanote-ci/live-rabbitmq-proof/` / `build-and-test-artifacts/live-rabbitmq-proof/`, и child-attributed combined bundle `.tmp/m015-s03-combined-proof/` / `build-and-test-artifacts/combined-proof/`. Для их регенерации используйте `bash scripts/ci/verify-m004-s03-live-kafka-proof.sh`, `bash scripts/ci/verify-m015-s02-live-rabbitmq-proof.sh` и `bash scripts/ci/verify-m015-s03-combined-report.sh`. Public support surface здесь additive: Kafka и RabbitMQ публикуют отдельные `yanote-async-report.json` / `yanote-async-report.html`, combined path публикует `yanote-combined-report.json` / `yanote-combined-report.html` с явными child-path ссылками, redaction-safe строками `binding support` / `declared semantics` / `runtime semantics`, но без hosted dashboard, без blended denominator и без broker-agnostic promise; raw retained headers и payload bodies в support bundle не запрашиваются.
...
- **Пройти widened async path и получить Kafka, RabbitMQ или combined proof bundles:** [`docs/guides/asyncapi-kafka.md`](docs/guides/asyncapi-kafka.md)
