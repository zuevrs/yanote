...
Yanote delivery through M011 is now closed as a completed milestone: repository maturity, Kafka/AsyncAPI coverage, async schema-depth validation, runtime/public-boundary hardening, HTTP payload-conformance depth, evidence-truth hardening, the supported core HTTP/Kafka contract-depth surface, and the widened HTTP request/media semantics are all implemented and backed by retained proof artifacts or surviving repo history. The published stable line is still `v1.0.128`, which remains the latest shipped release baseline; M011 now stands as the completed in-repo closeout for retained request-serialization, cookie, media, and format semantics.
