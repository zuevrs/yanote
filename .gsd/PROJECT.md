...
Yanote delivery through M012 is now closed as a completed milestone: repository maturity, Kafka/AsyncAPI coverage, async schema-depth validation, runtime/public-boundary hardening, HTTP payload-conformance depth, evidence-truth hardening, the supported core HTTP/Kafka contract-depth surface, widened HTTP request/media semantics, and the first truthful broader OpenAPI security semantics are all implemented and backed by retained proof artifacts or surviving repo history. The published stable line is still `v1.0.128`, which remains the latest shipped release baseline; M012 now stands as the completed in-repo closeout for additive HTTP security semantics and their public proof/support boundary.
...
- The next planned milestone is M013, which targets analyzer delivery/report UX improvements (remote spec loading, deprecated-operation handling, and operator-friendly report consumption) rather than more core contract rewrites.
