...
Yanote delivery through M010 is now closed in the codebase: repository maturity, Kafka/AsyncAPI coverage, async schema-depth validation, runtime/public-boundary hardening, HTTP payload-conformance depth, evidence-truth hardening, and the supported core HTTP/Kafka contract-depth surface are all complete and backed by retained proof artifacts or surviving repo history. The published stable line is now `v1.0.128`, which includes the M010 closeout plus the release/CI hardening needed to publish `yanote-recorder-spring-kafka` truthfully on Maven Central.
