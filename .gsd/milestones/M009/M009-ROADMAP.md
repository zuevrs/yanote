...
- HTTP/OpenAPI and Kafka/AsyncAPI report/gate surfaces keep observation separate from conformance while clearly distinguishing recorder-policy omission from true semantic drift.
...
- Contract verification: `yanote-core` JSONL round-trip tests, `yanote-js` spec/coverage/report/gate suites, recorder seam tests, and fixture-backed ambiguity / omission matrices.
...
- HTTP/OpenAPI and Kafka/AsyncAPI report/gate outputs expose recorder-policy omission distinctly from semantic contract drift.
...
- [ ] **S04: HTTP/OpenAPI recorder-policy and schema fidelity hardening** `risk:medium` `depends:[S01]`
...
Consumes:
- S02 retained header evidence and current async report/gate surfaces.
...
Consumes:
- S01 recorder provenance and current M008 HTTP payload/report/gate surfaces.
