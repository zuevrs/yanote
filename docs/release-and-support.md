...
- CI/public summary surface: `yanote-validation-artifacts` и `build-and-test-artifacts` публикуют отдельные HTTP-vs-async JSON+HTML report families, deterministic `artifact-manifest.txt` / `artifact-source-paths.txt`, sanitized `specSource` и additive deprecated-operation counts в GitHub step summaries без combined/dashboard wording;
