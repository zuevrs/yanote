...
- [`local-agent-workflow.md`](local-agent-workflow.md) — локальный root `AGENTS.md`, clone-local `.gsd/` / `.tmp/` / `.tmp-*` / `.vite/` boundary, repo-local `info/exclude` bootstrap и граница содержимого для maintainer-only workflow.
