...
3. [`guides/asyncapi-kafka.md`](guides/asyncapi-kafka.md) — widened async ветка: отдельные Kafka и RabbitMQ/AMQP proof families через `async-report`, retained `yanote-async-report.json` / `yanote-async-report.html`, child-attributed combined surface `yanote-combined-report.json` / `yanote-combined-report.html`, redaction-safe `binding support` / `declared semantics` / `runtime semantics`, явные bundle roots `live-kafka-proof/`, `live-rabbitmq-proof/`, `combined-proof/` и честная граница без blended denominator, hosted dashboard и broker-agnostic promise.
...
Эти четыре guide-level surface-а остаются каноническими, но concept-first HTTP onboarding по-прежнему первичен: для обычного recorder/analyzer цикла идите по пунктам 1 → 2 → 4, а если у вас есть widened async surface, ответвляйтесь в пункт 3 к отдельным Kafka и RabbitMQ proof families и к retained combined-report surface. Для async ветки авторитетными delivery surfaces считаются live Spring Kafka bundle `.yanote-ci/live-kafka-proof/`, первый RabbitMQ/AMQP bundle `.yanote-ci/live-rabbitmq-proof/` и child-attributed combined bundle `.tmp/m015-s03-combined-proof/`; в CI они публикуются как `build-and-test-artifacts/live-kafka-proof/`, `build-and-test-artifacts/live-rabbitmq-proof/` и `build-and-test-artifacts/combined-proof/`, где redaction-safe строками показываются `binding support`, `declared semantics`, `runtime semantics`, `protocols=amqp` и child report paths. Эта ветка additive: Kafka и RabbitMQ остаются отдельными `async-report` / `yanote-async-report.json` / `yanote-async-report.html` surface-ами, combined-report не смешивает HTTP и async numerators в один denominator и не превращается в hosted dashboard или broker-agnostic promise.
