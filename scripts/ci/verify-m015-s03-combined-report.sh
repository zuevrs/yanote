...
[[ -f "${RETAINED_ASYNC_REPORT_PATH}" ]] || fail "Missing retained async child report at ${RETAINED_ASYNC_REPORT_PATH}. Rerun bash scripts/ci/verify-m015-s02-live-rabbitmq-proof.sh to regenerate the canonical S02 async proof."
[[ -f "${RETAINED_ASYNC_HTML_PATH}" ]] || fail "Missing retained async child HTML at ${RETAINED_ASYNC_HTML_PATH}. Rerun bash scripts/ci/verify-m015-s02-live-rabbitmq-proof.sh to restore the canonical drill-down artifact."
