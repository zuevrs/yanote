...
test("yanote-recorder-spring-mvc keeps MVC APIs at compile time without exporting a servlet web stack at runtime", async () => {
  const [compileClasspath, runtimeClasspath] = await Promise.all([
    runGradle([":yanote-recorder-spring-mvc:dependencies", "--configuration", "compileClasspath", "--console=plain"]),
    runGradle([":yanote-recorder-spring-mvc:dependencies", "--configuration", "runtimeClasspath", "--console=plain"])
  ]);
  ...
  assert.match(compileClasspath.stdout, /org\.springframework:spring-webmvc:6\.1\.3/);
  assert.match(compileClasspath.stdout, /jakarta\.servlet:jakarta\.servlet-api:6\.0\.0/);

  assert.doesNotMatch(runtimeClasspath.stdout, /org\.springframework\.boot:spring-boot-starter-web:/);
  assert.doesNotMatch(runtimeClasspath.stdout, /org\.springframework\.boot:spring-boot-starter-tomcat:/);
