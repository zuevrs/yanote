...
if [[ "${boundary_doc_present}" == "true" ]]; then
  ...
  require_contains "${BOUNDARY_DOC}" "request-semantics.events.jsonl" "request sidecar artifact"
  require_contains "${BOUNDARY_DOC}" "request-semantics.stdout" "request sidecar artifact"
  require_contains "${BOUNDARY_DOC}" "request-semantics.stderr" "request sidecar artifact"
  require_contains "${BOUNDARY_DOC}" "request-semantics-yanote-report.json" "request sidecar artifact"
  require_contains "${BOUNDARY_DOC}" "bash scripts/ci/verify-m011-s02-request-semantics.sh" "focused request proof command"
  require_contains "${BOUNDARY_DOC}" "bash scripts/ci/verify-m011-s03-format-media.sh" "focused payload proof command"
  require_contains "${BOUNDARY_DOC}" "httpRequestConformance" "request report contract surface"
  require_contains "${BOUNDARY_DOC}" "declaredSupport" "request support field"
  require_contains "${BOUNDARY_DOC}" "declaredSupportShape" "request support field"
  require_contains "${BOUNDARY_DOC}" "declaredSupportReason" "request support field"
  require_contains "${BOUNDARY_DOC}" "request_observed_operations" "request summary token"
  require_contains "${BOUNDARY_DOC}" "request_observed_parameters" "request summary token"
  require_contains "${BOUNDARY_DOC}" "request_truths" "request summary token"
  require_contains "${BOUNDARY_DOC}" "path=simple" "request subset wording"
  require_contains "${BOUNDARY_DOC}" "query=form" "request subset wording"
  require_contains "${BOUNDARY_DOC}" "header=simple" "request subset wording"
  require_contains "${BOUNDARY_DOC}" "cookie=form" "request subset wording"
  require_contains "${BOUNDARY_DOC}" 'query=form` + `explode=true` + scalar `items`' "repeated query-array wording"
  require_contains "${BOUNDARY_DOC}" "SEMANTIC_HTTP_UNSUPPORTED_REQUEST_PARAMETER" "request fail-closed wording"
  ...
  require_contains "${BOUNDARY_DOC}" "HTTP Request Conformance" "request conformance wording"
  require_contains "${BOUNDARY_DOC}" "JSON-first request/response payload validation" "JSON-first boundary wording"
  require_contains "${BOUNDARY_DOC}" '`email`-only' "email-only format wording"
  require_contains "${BOUNDARY_DOC}" "most-specific declared match" "media specificity wording"
...
require_contains "${ROOT_README}" "request-semantics.events.jsonl" "root landing request sidecar"
require_contains "${ROOT_README}" "SEMANTIC_HTTP_UNSUPPORTED_REQUEST_PARAMETER" "root landing request fail-closed wording"
...
require_contains "${DOCS_README}" "path=simple" "docs landing request subset wording"
require_contains "${DOCS_README}" '`email`-only payload format allowlist' "docs landing payload boundary wording"
require_contains "${ANALYZER_GUIDE}" "request-semantics.events.jsonl" "analyzer guide request sidecar wording"
require_contains "${ANALYZER_GUIDE}" "SEMANTIC_HTTP_UNSUPPORTED_REQUEST_PARAMETER" "analyzer guide request fail-closed wording"
...
echo "S04 boundary verification passed: release/support surface, retained request/payload proof wording, and version-source disclaimers align with ${latest_tag} (${release_line})."
